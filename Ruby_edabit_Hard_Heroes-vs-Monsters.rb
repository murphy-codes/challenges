=begin
  Author: Tom Murphy
  Last Modified: 2019-11-22 13:30
=end

# https://edabit.com/challenge/R9LbWMxgd84fEtYHK
...
=begin
  ...
  Given a hash containing the character's stats (with p-prefix ones being the hero's stats and m-prefix ones being the monster's) you must return a string:
...
def battle(stats)
  ...
  while continue
    if turn # player's turn
      if stats["pHP"] <= 10 && stats["pPOT"] > 0
        stats["pHP"]+=10
        stats["pPOT"]-=1
        heal = true
      else
        damage = (2*stats["pATT"])-stats["mDEF"]
        if damage > 0
          stats["mHP"] -= damage
        end
      end
      ...
      if heal
        damage = ((2*stats["mATT"])-stats["pDEF"])/2
        if damage > 0
          stats["pHP"] -= damage
        end
        heal = false
      else
        damage = (2*stats["mATT"])-stats["pDEF"]
        if damage > 0
          stats["pHP"] -= damage
        end
      end
  ...
  if victory
    result = "Victory in " + round.to_s + " round"
  else
    result = "Game Over in " + round.to_s + " round"
  end
  if round > 1
    result += "s"
  end
