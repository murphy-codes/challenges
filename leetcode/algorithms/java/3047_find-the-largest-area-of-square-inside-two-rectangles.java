...
* _ axis. You are given two 2D integer arrays `bottomLeft` and `topRight`
* _ where `bottomLeft[i] = [a_i, b_i]` and `topRight[i] = [c_i, d_i]` represent
* _ the bottom-left and top-right coordinates of the `i^th` rectangle, respectively.
* You need to find the maximum area of a square that can fit inside the
* _ intersecting region of at least two rectangles. Return `0` if such a
* _ square does not exist.
*
* Example 1:
