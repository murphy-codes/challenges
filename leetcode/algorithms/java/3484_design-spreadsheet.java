// Source: https://leetcode.com/problems/design-spreadsheet/
...
* _ given number of `rows`. Each cell in the spreadsheet can hold an integer
* _ value between 0 and 10^5.
* Implement the `Spreadsheet` class:
* • `Spreadsheet(int rows)` Initializes a spreadsheet with 26 columns
* __ (labeled `'A'` to `'Z'`) and the specified number of rows. All cells are
* __ initially set to 0.
* • `void setCell(String cell, int value)` Sets the value of the specified `cell`.
* __ The cell reference is provided in the format `"AX"` (e.g., `"A1"`, `"B10"`),
* __ where the letter represents the column (from `'A'` to `'Z'`) and the number
* __ represents a 1-indexed row.
* • `void resetCell(String cell)` Resets the specified cell to 0.
* • `int getValue(String formula)` Evaluates a formula of the form `"=X+Y"`,
* __ where `X` and `Y` are either cell references or non-negative integers, and
* __ returns the computed sum.
* Note: If `getValue` references a cell that has not been explicitly set using
* _ `setCell`, its value is considered `0`.
*
* Example 1:
* Input:
