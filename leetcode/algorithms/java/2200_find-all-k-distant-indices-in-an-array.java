...
class Solution {
    // Scans the array to find each occurrence of the key, then adds all indices
    // within k distance of that occurrence. Uses a sliding start index to avoid
    // duplicates and redundant checks. Time complexity is O(n), where n is the
    // array length. Space complexity is O(m), where m is the result size.
    static { // JIT warm-up
